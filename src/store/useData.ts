...
export const useData = create<DataState>((set, get) => ({
  ...
  loadAll: async () => {
    ...
    const [projects, phases, contractors, activeProjectId] = await Promise.all([
      ipc['projects:list'](),
      ipc['phases:list'](),
      ipc['contractors:list'](),
      ipc['projects:getActive'](),
    ]);
    const subphases: Record<string, Subphase[]> = {};
    await Promise.all(
      phases.map(async (p: Phase) => {
        subphases[p.id] = await ipc['subphases:list'](p.id);
      })
    );
  ...
  setActiveProject: async (id: string) => {
    await ipc['projects:setActive'](id);
    const costs = await ipc['costs:list']({ projectId: id });
    const documents = await ipc['documents:listByProject'](id);
    set({ activeProjectId: id, costs, documents });
  },
  addProject: async (payload) => {
    const project = await ipc['projects:create'](payload);
    await get().loadAll();
    return project;
  },
  addCost: async (payload) => {
    const cost = await ipc['costs:create'](payload);
    const costs = await ipc['costs:list']({ projectId: payload.project_id });
    set({ costs });
    return cost;
  },
  refreshDocuments: async (projectId) => {
    const documents = await ipc['documents:listByProject'](projectId);
    set({ documents });
  },
  addPhase: async (name: string) => {
    await ipc['phases:create'](name);
    const phases = await ipc['phases:list']();
    const subphases: Record<string, Subphase[]> = {};
    await Promise.all(
      phases.map(async (p: Phase) => {
        subphases[p.id] = await ipc['subphases:list'](p.id);
